...
M.setup = function()
  ...
  -- translate
  keymap("n", "<leader>tz", ":Translate ZH -source=EN -parse_after=window -output=floating<cr>", opts)
  keymap("v", "<leader>tz", ":Translate ZH -source=EN -parse_after=window -output=floating<cr>", opts)
  keymap("n", "<leader>te", ":Translate EN -source=ZH -parse_after=window -output=floating<cr>", opts)
  keymap("v", "<leader>te", ":Translate EN -source=ZH -parse_after=window -output=floating<cr>", opts)

  -- buffer %bd 删除所有缓冲区, e# 打开最后一个缓冲区, bd# 关闭[No Name]
  vim.api.nvim_create_user_command("BufferCloseOther", function()
    require("hon6.bsc.utils").close_other_bufline()
  end, {})
  ...
  vim.keymap.set("v", "<leader>tt", function()
    ...
    local text = require("hon6.bsc.utils").get_visual_selection()
  ...
  -- camel_case
  require("hon6.bsc.utils").camel_case_init()
...
M.maplsp = function(client, buffer)
  ...
  bkeymap(buffer, "n", "K", "<cmd>lua vim.lsp.buf.hover()<CR>", opts)
  -- rename
  bkeymap(buffer, "n", "<leader>r", "<cmd>lua vim.lsp.buf.rename()<CR>", opts)
  -- code action
  bkeymap(buffer, "n", "<leader>a", "<cmd>lua vim.lsp.buf.code_action()<CR>", opts)
  bkeymap(buffer, "v", "<leader>a", "<cmd>lua vim.lsp.buf.code_action()<CR>", opts)
  -- go xx
  -- bkeymap('n', 'gd', '<cmd>lua vim.lsp.buf.definition()<CR>', opts)
  -- vim.api.nvim_buf_set_keymap(bufnr, 'n', 'gd', '<cmd>Trouble lsp_definitions<CR>', opts)
  bkeymap(buffer, "n", "gd", "<cmd>Telescope lsp_definitions<CR>", opts)
  bkeymap(buffer, "n", "gh", "<cmd>lua vim.lsp.buf.hover()<CR>", opts)
  bkeymap(buffer, "n", "gs", "<cmd>lua vim.lsp.buf.signature_help()<CR>", opts)
  -- bkeymap('n', 'gD', '<cmd>lua vim.lsp.buf.declaration()<CR>', opts)
  -- bkeymap('n', 'gD', '<cmd>lua vim.lsp.buf.type_definition()<CR>', opts)
  -- bkeymap(bufnr, "n", "gD", "<cmd>Trouble lsp_type_definitions<CR>", opts)
  -- bkeymap('n', 'gi', '<cmd>lua vim.lsp.buf.implementation()<CR>', opts)
  -- vim.api.nvim_buf_set_keymap(bufnr, "n", "gi", "<cmd>Trouble lsp_implementations<CR>", opts)
  bkeymap(buffer, "n", "gi", "<cmd>Telescope lsp_implementations<CR>", opts)
  -- bkeymap('n', 'gr', '<cmd>lua vim.lsp.buf.references()<CR>', opts)
  -- vim.api.nvim_buf_set_keymap(bufnr, "n", "gr", "<cmd>Trouble lsp_references<CR>", opts)
  bkeymap(buffer, "n", "gr", "<cmd>Telescope lsp_references<CR>", opts)
  -- bkeymap('n', 'gr', '<cmd>lua require"lspsaga.provider".lsp_finder()<CR>', opts)
  -- bkeymap('n', '<space>s', '<cmd>lua vim.lsp.buf.workspace_symbol()<CR>', opts)
  -- bkeymap('n', '<space>s', '<cmd>lua require"telescope.builtin".lsp_workspace_symbols({ query = vim.fn.input("Query> ") })<CR>', opts)
  bkeymap(buffer, "n", "<leader>fs", "<cmd>Telescope lsp_dynamic_workspace_symbols<CR>", opts)
  ...
  bkeymap(buffer, "n", "<leader>fo", "<cmd>Telescope lsp_document_symbols<CR>", opts)

  -- diagnostic
  bkeymap(buffer, "n", "go", "<cmd>lua vim.diagnostic.open_float()<CR>", opts)
  bkeymap(buffer, "n", "[g", "<cmd>lua vim.diagnostic.goto_prev()<CR>", opts)
  bkeymap(buffer, "n", "]g", "<cmd>lua vim.diagnostic.goto_next()<CR>", opts)
  bkeymap(
    buffer,
    "n",
    "[e",
  ...
  -- bkeymap('n', '<leader>q', '<cmd>lua vim.diagnostic.setloclist()<CR>', opt)
  -- leader + =
  ...
  vim.keymap.set("n", "<leader>=", function()
    ...
    vim.lsp.buf.format({
      bufnr = bfn,
      filter = function(c)
        return require("hon6.lsp.utils").filter_format_lsp_client(c, bfn)
      end,
    })
  end, opts)
  ...
  bkeymap(
    ...
    "v",
    "<leader>=",
    '<cmd>lua require("hon6.lsp.utils").format_range_operator()<CR>',
    opts
  )

  -- bkeymap('v', '<leader>=', '<cmd>lua vim.lsp.buf.formatting()<CR>', opts)
  -- bkeymap('n', '<C-k>', '<cmd>lua vim.lsp.buf.signature_help()<CR>', opts)
  -- bkeymap('n', '<space>wa', '<cmd>lua vim.lsp.buf.add_workspace_folder()<CR>', opts)
  -- bkeymap('n', '<space>wr', '<cmd>lua vim.lsp.buf.remove_workspace_folder()<CR>', opts)
  -- bkeymap('n', '<space>wl', '<cmd>lua print(vim.inspect(vim.lsp.buf.list_workspace_folders()))<CR>', opts)
  -- bkeymap('n', '<space>D', '<cmd>lua vim.lsp.buf.type_definition()<CR>', opts)

  bkeymap(buffer, "n", "<leader>xw", "<cmd>Telescope diagnostics<CR>", opts)
  bkeymap(
    buffer,
    "n",
    "<leader>xe",
  ...
  -- bkeymap(bufnr, "n", "<leader>xw", "<cmd>Trouble workspace_diagnostics<CR>", opts)
  -- bkeymap(bufnr, "n", "<leader>xx", "<cmd>Trouble<CR>", opts)
  -- bkeymap(bufnr, "n", "<leader>xw", "<cmd>Trouble workspace_diagnostics<CR>", opts)
  -- bkeymap(bufnr, "n", "<leader>xd", "<cmd>Trouble document_diagnostics<CR>", opts)
  -- bkeymap(bufnr, "n", "<leader>xq", "<cmd>Trouble quickfix<CR>", opts)
  ...
  if codeLensProvider then
    bkeymap(buffer, "n", "<leader>cr", "<Cmd>lua vim.lsp.codelens.refresh()<CR>", opts)
    bkeymap(buffer, "n", "<leader>ce", "<Cmd>lua vim.lsp.codelens.run()<CR>", opts)
  end
end
